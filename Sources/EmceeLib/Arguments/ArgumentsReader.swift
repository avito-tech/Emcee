...
final class ArgumentsReader {
    ...
    public static func environment(_ file: String?, key: SPMArgumentDescription) throws -> [String: String] {
        return try decodeModelsFromFile(file, defaultValueIfFileIsMissing: [:], key: key, jsonDecoder: strictDecoder)
    }
    
    public static func testArgFile(_ file: String?, key: SPMArgumentDescription) throws -> TestArgFile {
        return try decodeModelsFromFile(file, key: key, jsonDecoder: strictDecoder)
    }
    
    public static func testDestinations(_ file: String?, key: SPMArgumentDescription) throws -> [TestDestinationConfiguration] {
        return try decodeModelsFromFile(file, key: key, jsonDecoder: decoderWithSnakeCaseSupport)
    }
    
    public static func deploymentDestinations(_ file: String?, key: SPMArgumentDescription) throws -> [DeploymentDestination] {
        return try decodeModelsFromFile(file, key: key, jsonDecoder: decoderWithSnakeCaseSupport)
    }
    
    public static func simulatorSettings(
        localizationFile: String?,
        localizationKey: SPMArgumentDescription,
        watchdogFile: String?,
        watchdogKey: SPMArgumentDescription
    ...
    public static func socketAddress(_ value: String?, key: SPMArgumentDescription) throws -> SocketAddress {
        let stringValue = try validateNotNil(value, key: key)
        return try SocketAddress.from(string: stringValue)
    }
    
    public static func validateNotNil<T>(_ value: T?, key: SPMArgumentDescription) throws -> T {
        guard let value = value else { throw ArgumentsError.argumentIsMissing(key) }
        return value
    }
    
    public static func validateResourceLocation(_ value: String?, key: SPMArgumentDescription) throws -> ResourceLocation {
        let string = try validateNotNil(value, key: key)
        return try ResourceLocation.from(string)
    }
    
    public static func validateResourceLocationOrNil(_ value: String?, key: SPMArgumentDescription) throws -> ResourceLocation? {
        guard let string = value else { return nil }
        return try ResourceLocation.from(string)
    }
    
    public static func validateResourceLocations(_ values: [String], key: SPMArgumentDescription) throws -> [ResourceLocation] {
    ...
    public static func validateFileExists(_ value: String?, key: SPMArgumentDescription) throws -> String {
